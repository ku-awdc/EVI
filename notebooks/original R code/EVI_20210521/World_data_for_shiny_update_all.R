...
wdata=read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv")

wdata=as.data.frame(wdata)
wdata$Country.Region=as.factor(wdata$Country.Region)
wdata$loc_n=as.numeric(wdata$Country.Region)
nn=length(levels(wdata$Country.Region))
last_c=ncol(wdata)-1
wdata_l <- gather(wdata, condition, measurement, 5:last_c, factor_key=TRUE)
...
for(i in 1:nn) {
  ...
  nam[i] <- paste(wdata_ll$Country.Region[1], ".csv", sep = "")
  ...
  write.table(wdata_ll, file=file.path("W_cc_n",nam[i]), sep=";", append=FALSE , 
...
for(k in 1:nn) {
  ...
  datal_old <- read_delim(paste("W_cc/",names[k,], sep=""),";", escape_double = FALSE, trim_ws = TRUE)
  
  datal <- read_delim(paste("W_cc_n/",names[k,], sep=""),";", escape_double = FALSE, trim_ws = TRUE)
  ...
  for (i in (nrow(datal_old)+1): length(cases)){
    ...
    write.table(data, file=file.path("W_cc_n/results",namesa), sep=",", append=T , 
...
for(i in 1:nn) {
  ...
  results_old <- read_delim(paste("/srv/shiny-server/forplot_n/",namesa, sep=""),",",escape_double = FALSE, trim_ws = TRUE)
  results_new <- read_delim(paste("W_cc_n/results/",namesa, sep=""),",",escape_double = FALSE, trim_ws = TRUE, col_names =F)
  
 
  write.table(results_old, file=file.path("W_cc_upd",namesa), sep=",", append=F , 
              quote=F, row.names=FALSE, col.names=T)
  write.table(results_new, file=file.path("W_cc_upd",namesa), sep=",", append=T , 
              quote=F, row.names=FALSE, col.names=F)
...
my_function <- function(x){
  file.rename( from = file.path("W_cc_upd", x) ,
               to = file.path("/srv/shiny-server/forplot_n", x) )
}
...
my_function2 <- function(x){
  file.rename( from = file.path("W_cc_n", x) ,
               to = file.path("W_cc", x) )
}
...
my_function3 <- function(x){
  file.rename( from = file.path("W_cc_n/results", x) ,
               to = file.path("W_cc_throw", x) )
}
...
for(i in 1:nn) {
  namesa <- paste("results", names[i,], sep = "")
  find_c <- read_delim(paste("/srv/shiny-server/forplot_n/",namesa, sep=""),",",escape_double = FALSE, trim_ws = TRUE)
  find_c=as.data.frame(find_c)
  find_c=tail(find_c, n=5)
  an=mean(find_c$Index)
  res=as.data.frame(cbind(an,names[i,]))
  
  write.table(res, file=file.path("W_cc_find","finder"), sep=",", append=T , 
              quote=F, row.names=FALSE, col.names=F)
