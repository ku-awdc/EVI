...
for(i in 1:nn) {
  ...
  write.table(wdata_ll, file=file.path("W_cc_corr",nam[i]), sep=";", append=FALSE , 
...
for(k in 1:nn) {
  ...
  datal <- read_delim(paste("W_cc/",names[k,], sep=""),";",escape_double = FALSE, trim_ws = TRUE)
  ...
  write.table(data, file=paste("forplot_corr/",namesa, sep=""), sep=",", append=F , 
              quote=F, row.names=FALSE, col.names=T)
